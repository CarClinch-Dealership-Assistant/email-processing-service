...
* **Durable Orchestrator:** A long-running function that handles the configurable 24/48/72-hour sleep cycles using virtual timers
...
4.  **Action:** If all checks pass, it generates a sequence-specific email (ex. Sequence #2 includes alternative vehicles). If any check fails, the orchestrator self-terminates.
